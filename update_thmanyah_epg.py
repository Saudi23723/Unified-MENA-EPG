...
EXTRA_SOURCES = (
    {
        ...
        "keywords": ("مباريات اليوم", "القنوات الناقلة", "مواعيد مباريات"),
...
YALLAKORA_CARD_RE = re.compile(r"(.+?)\1\s*[-–—\s]*?(\d{1,2}:\d{2})\s*(.+?)\3\s*$")
...
def collect_yallakora_fixtures():
    ...
    for offset in range(0, EXTRA_LOOKAHEAD_DAYS + 1):
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            hour, minute = card.group(2).split(":")
            title = f"{home} - {away}"
            key = (day, title)
            if key in seen:
                continue
        ...
        if found:
            log(f"Yallakora fixtures {day}: {found}")
