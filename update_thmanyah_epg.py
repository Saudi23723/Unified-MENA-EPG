...
def write_xml(events):
    ...
    def time_text(event):
        ...
        vegas = event["start"].astimezone(LAS_VEGAS_TZ)
        return f"أبو ظبي {abu:%H:%M} | لاس فيغاس {vegas:%H:%M}"
...
def main():
    log("THMANYAH FINAL v5 | NO OCR | NO FALLBACK | GUIDE ENABLED | ABU DHABI + LAS VEGAS")
