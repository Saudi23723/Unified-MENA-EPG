...
def collect_radarkora_confirmations(daily):
    """
    RadarKora confirmation source with Telegram pagination.

    It walks backward through multiple public Telegram preview pages using
    ?before=<message_id>, looking for #مباريات_اليوم posts that fall inside
    our active date window. Only explicit 'ثمانية N' labels count as channel
    confirmations. No channel number is ever guessed.
    """
    confirmations = []

    # Search backward far enough to cover the 10-day EPG window even when
    # RadarKora posts many messages per day.
    max_pages = 12
    before_id = None
    seen_message_ids = set()
    pages_scanned = 0
    schedule_posts_seen = 0
    ...
    for page_index in range(max_pages):
        if before_id is None:
            page_url = RADARKORA_TELEGRAM
        else:
            page_url = f"{RADARKORA_TELEGRAM}?before={before_id}"
        ...
        if not posts:
            log(
                f"RadarKora pagination stopped: no posts on page {page_index + 1}"
            )
            break
        ...
        for post in posts:
            ...
            schedule_posts_seen += 1

            post_day = radar_post_day(caption_norm)
            if not post_day:
                log(
                    "RadarKora schedule post skipped: "
                    "could not determine date from caption"
                )
                continue
            ...
            if not image_url:
                log(
                    f"RadarKora {post_day}: schedule post has no readable image URL"
                )
                continue

            ocr_text = ocr_image_url(image_url)
            if not ocr_text:
                log(f"RadarKora {post_day}: OCR returned no text")
                continue
            ...
            log(
                ...
                f"{len(contexts)} explicit Thmanyah channel labels found by OCR"
            )
            ...
            for channel, context in contexts:
                ...
                # Conservative acceptance: both teams need evidence and the
                # best match must clearly beat the runner-up.
                if best_score < 1.0:
                    continue
        ...
        if not page_ids:
            log(
                f"RadarKora pagination stopped: "
                f"no message IDs found on page {page_index + 1}"
            )
            break

        # Telegram preview pages are reverse chronological. The smallest
        # message ID on this page moves us further back without repeating it.
        next_before = min(page_ids)

        if before_id is not None and next_before >= before_id:
            log("RadarKora pagination stopped: no backward progress")
            break

        before_id = next_before

        # Once a page contains dated schedule posts that are all older than
        # our active window, there is no reason to crawl further backward.
        if page_days and max(page_days) < window_floor:
            log(
                "RadarKora pagination reached posts older than active window; "
                "stopping."
            )
            break
    ...
    log(
        "RadarKora pagination summary | "
        f"pages={pages_scanned}, "
        f"schedule_posts={schedule_posts_seen}, "
        f"ocr_images={ocr_images_processed}, "
        f"numbered_confirmations={len(confirmations)}"
    )
