...
def write_xml(events):
    ...
    for event in events:
        ...
        if channel_number in CHANNELS:
            ...
            desc = (
                "المباراة معلنة على شبكة ثمانية، "
                "لكن رقم القناة 1/2/3 غير مؤكد حتى الآن. "
                f"موعد المباراة: {event['start']:%Y-%m-%d %H:%M} بتوقيت مكة. "
                f"المصدر: {event['source']}"
            )
    ...
    def add_hourly_filler(channel_id, gap_start, gap_stop):
        ...
        while cursor < gap_stop:
            ...
            if channel_id == GUIDE_CHANNEL_ID:
                filler_desc = (
                    "لا توجد حالياً مباراة على شبكة ثمانية "
                    "بانتظار تحديد رقم القناة."
                )
            else:
                filler_desc = (
    ...
    for channel_id, intervals in real_by_id.items():
        ...
        for start, stop in sorted(intervals):
            ...
            if stop > start:
                clean_intervals.append((start, stop))

        merged_intervals = []

        for start, stop in clean_intervals:
            if (
                not merged_intervals
                or start > merged_intervals[-1][1]
            ):
    ...
    for channel in root.findall("channel"):
        ...
        if (
            icon is None
            or icon.get("src") != THMANYAH_LOGO
        ):
            raise RuntimeError(
                "Thmanyah XML validation failed; "
                f"missing logo on {channel.get('id')}"
            )
