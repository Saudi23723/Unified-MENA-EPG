...
def assign_unconfirmed(events):
    """
    No fourth Guide channel.
    For each exact kickoff time:
      - preserve explicit channel numbers first
      - assign unnumbered fixtures to remaining free channels 1/2/3
    """
    grouped = defaultdict(list)

    for event in events:
        slot = event["start"].strftime("%Y%m%d%H%M")
        grouped[slot].append(dict(event))
    ...
    for slot in sorted(grouped):
        ...
        used = {int(item["channel"]) for item in confirmed_items}
        output.extend(confirmed_items)

        free = [n for n in CHANNELS if n not in used]

        unconfirmed_items = sorted(
            unconfirmed_items,
            key=lambda item: norm(item["title"]).casefold(),
        )

        for item in unconfirmed_items:
            if not free:
                warn(
                    "THMANYAH overflow dropped | "
            ...
            item["source"] = (
                f"{item['source']} + fallback slot assignment"
            )
            output.append(item)

            log(
                "THMANYAH FALLBACK ASSIGNMENT | "
                f"{item['start']:%Y-%m-%d %H:%M} | "
                f"{item['title']} | THMANYAH {number}"
            )
...
def merge_existing(existing, fresh):
    ...
    for event in fresh:
        ...
        old_confirmed = bool(old.get("confirmed"))
        new_confirmed = bool(event.get("confirmed"))
...
def write_xml(events):
    tv = ET.Element(
        "tv",
        {"generator-info-name": "Thmanyah 1-2-3 Sports EPG"},
    )
    ...
    for event in events:
        ...
        if event.get("confirmed"):
            assignment_text = f"القناة المعلنة: ثمانية {number}."
        else:
            assignment_text = (
                f"تم وضع المباراة على ثمانية {number} تلقائياً "
                "لأن المصدر لم يذكر رقم القناة."
            )
    ...
    if set(channel_ids) != expected_ids or len(channel_ids) != 3:
        raise RuntimeError(
            "Thmanyah XML validation failed; expected exactly 3 channels, got: "
            + ", ".join(channel_ids)
        )

    for channel in root.findall("channel"):
        icon = channel.find("icon")
        if icon is None or icon.get("src") != THMANYAH_LOGO:
            raise RuntimeError(
                f"Missing Thmanyah logo on {channel.get('id')}"
            )
...
def main():
    ...
    fresh = [
        ...
        and in_window(event["start"])
    ]

    log(f"Thmanyah newly resolved programmes: {len(fresh)}")

    for event in fresh:
        marker = "CONFIRMED" if event.get("confirmed") else "FALLBACK"
        log(
            f"  THMANYAH {event['channel']} [{marker}] | "
