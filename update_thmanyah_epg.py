...
def main():
    ...
    # Always rewrite the XML so channel definitions and logo are refreshed.
    # If fresh verified data exists, use it.
    # If sources temporarily return zero, preserve existing valid programmes.
    if fresh:
        final_events = dedupe(fresh)
        log(f"Thmanyah total programmes written: {len(final_events)}")
    else:
        final_events = dedupe(old)
        if old:
            warn(
                "No fresh Thmanyah data; preserving existing programmes "
                "while refreshing channel definitions and logo"
            )
        else:
            warn(
                "No fresh Thmanyah data and no existing programmes; "
                "writing channel definitions only"
            )
