...
def write_xml(events):
    ...
    tv = ET.Element("tv", {"generator-info-name": "Thmanyah Sports verified EPG"})
...
def main():
    ...
    merged = merge_existing(old, fresh)
    log(f"Thmanyah total programmes after merge: {len(merged)}")

    # Do not destroy a previously useful file on a temporary source outage.
