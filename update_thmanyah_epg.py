...
def collect_radarkora_confirmations(daily):
    ...
    for page_index in range(max_pages):
        ...
        for post in posts:
            ...
            rows = radar_rows_from_ocr(ocr_text, fallback_day=post_day)
            ...
            for row_day, channel, context in rows:
                if not row_day or not (window_floor <= row_day <= window_ceiling):
                    continue
                ...
                confirmations.append({
                    ...
                    "title": best["title"],
                    "source": "matches_today2 Telegram OCR",
                    "confirmed": True,
                })
...
def apply_confirmations(daily, confirmations):
    ...
    for event in daily:
        ...
        if len(channels) == 1:
            ...
            current["confirmed"] = True
            current["source"] = (
                f"{current['source']} + 365Scores channel-only confirmation"
            )

            # Hard safety invariant: channel confirmation may never alter time.
            if current["start"] != original_start:
                raise RuntimeError("365Scores confirmation altered kickoff time")

            log(
                "365 CHANNEL CONFIRMATION | "
...
def write_xml(events):
    tv = ET.Element(
        "tv",
        {"generator-info-name": "Thmanyah Sports EPG FINAL v8"},
    )
...
def main():
    log("THMANYAH FINAL v10 | matches_today2 IMAGE-DATE OCR | SOURCE TIME ONLY | 365 + Telegram CHANNEL-ONLY | TIVIMATE AUTO-CONVERT | NO ABU DHABI/LAS VEGAS")
    ...
    confirmations_telegram = collect_radarkora_confirmations(daily)
