...
def read_existing():
    ...
    for programme in root.findall("programme"):
        ...
        if not pending and not channel_match:
            continue
        ...
        events.append({
            "channel": None if pending else int(channel_match.group(1)),
            "start": start,
            "title": title,
            "source": "existing XML",
            "channel_status": "unannounced" if pending else "confirmed",
        })
...
def write_xml(events):
    ...
    ET.SubElement(
        ...
        "display-name",
        {"lang": "ar"},
    ).text = "ثمانية - القناة لم تُعلن بعد"
    ET.SubElement(
        pending_channel,
        "display-name",
        {"lang": "en"},
    ).text = "Thmanyah - Channel TBA"
    ET.SubElement(
        pending_channel,
        "icon",
        {"src": THMANYAH_LOGO},
