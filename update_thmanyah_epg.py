...
def collect_radarkora_confirmations(daily):
    ...
    for page_index in range(max_pages):
        ...
        for post in posts:
            ...
            # Run BOTH OCR strategies and merge them.
            ...
            for row_day, channel, context in rows:
                ...
                if not scored:
                    log(
                        "OCR ROW UNMATCHED | "
                        f"{row_day} | THMANYAH {channel} | {normalize_ocr(context)[:180]}"
                    )
                    continue
...
def write_xml(events):
    tv = ET.Element(
        "tv",
        {"generator-info-name": "Thmanyah Sports EPG FINAL v12"},
    )
...
def main():
    log("THMANYAH FINAL v13 | OCR AUTHORITATIVE TEAM+CHANNEL | DUAL OCR | SOURCE TIME ONLY | 365 BACKUP | TIVIMATE AUTO-CONVERT")
