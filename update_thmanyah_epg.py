...
def _structured_rows_from_image(image, fallback_day=None):
    ...
    for y, line_text in lines:
        ...
        # Fast path: the full-line TSV already contains most rows. Only OCR a
        # specific column when that field is missing/weak.
        channel_fast = _channel_from_middle_column(line_text)
        if channel_fast not in CHANNELS:
            for psm in (7, 11):
                try:
        ...
        channel = channel_fast if channel_fast in CHANNELS else _channel_from_middle_column(middle_text + " | " + line_text)
        if channel not in CHANNELS:
            continue
...
def collect_radarkora_confirmations(daily):
    ...
    for page_index in range(max_pages):
        ...
        for post in posts:
            ...
            # v15: first parse the table by its actual visual columns. These rows
            # contain date + kickoff + teams + numbered channel, so they are
            # complete EPG events and do NOT need a Goal/Kooora article to exist.
            structured_rows = ocr_structured_table_url(
                image_url,
                fallback_day=post_day,
            )
            if structured_rows:
                log(f"matches_today2 STRUCTURED TABLE: {len(structured_rows)} complete numbered rows")
            for srow in structured_rows:
                if not in_window(srow["start"]):
                    continue
                direct_events.append({
                    "start": srow["start"],
                    "title": srow["title"],
                    "channel": srow["channel"],
                    "source": "matches_today2 structured image OCR",
                    "confirmed": True,
                })
                ...
                log(
                    "STRUCTURED OCR DIRECT EVENT | "
                    f"{srow['start']:%Y-%m-%d %H:%M} | "
                    f"{srow['title']} | THMANYAH {srow['channel']}"
                )
...
def main():
    log("THMANYAH FINAL v16 | TIGHT ROW OCR DATE+TIME+TEAM+CHANNEL | DUAL OCR BACKUP | 365 BACKUP | TIVIMATE AUTO-CONVERT")
    ...
    confirmations_telegram, telegram_direct_events = collect_radarkora_confirmations(daily)

    # Complete Telegram table rows carry their own Saudi date/time/team/channel.
    # Merge them into the fixture pool so future-day rows are not lost merely
    # because Goal/Kooora has not published a daily article yet.
    daily = dedupe(daily + telegram_direct_events)
