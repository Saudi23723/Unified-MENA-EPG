...
def dedupe(events):
    ...
    for event in sorted(
        events,
        key=lambda item: (item["start"], int(item["channel"]), item["title"]),
    ):
        key = event_key(event)
        if key in seen:
            continue
...
def resolve_events(daily, confirmations):
    ...
    for event in daily:
        ...
        candidates = [
            ...
            for candidate in candidates
            if abs(
                (candidate["start"].date() - event["start"].date()).days
            ) <= 1
        ]
        ...
        if candidates:
            best = min(
                ...
                key=lambda candidate: abs(
                    (candidate["start"] - event["start"]).total_seconds()
                ),
            )
            resolved = dict(event)
            resolved["channel"] = best["channel"]
            resolved["source"] = f"{event['source']} + {best['source']}"
...
def write_xml(events):
    observed_channels = {int(event["channel"]) for event in events}
    channels = sorted(DEFAULT_CHANNELS | observed_channels)

    tv = ET.Element(
        "tv",
        {"generator-info-name": "Thmanyah Sports Multi-Source EPG"},
    )
