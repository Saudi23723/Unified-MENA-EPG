...
EXTRA_SOURCES = (
    {
        ...
        "keywords": ("مباريات اليوم", "القنوات الناقلة"),
        "tz": ZoneInfo("Africa/Cairo"),
    },
    {
        "label": "Yallakora",
        "home": "https://www.yallakora.com/matches-center",
        "keywords": ("مباريات اليوم", "القنوات الناقلة"),
        "tz": ZoneInfo("Africa/Cairo"),
    },
    {
        "label": "Youm7",
        "home": "https://www.youm7.com/Sport",
        "keywords": ("مواعيد مباريات اليوم", "القنوات الناقلة"),
        "tz": ZoneInfo("Africa/Cairo"),
    },
    {
        "label": "Masrawy",
    ...
    },
    {
        ...
        "keywords": ("القنوات الناقلة", "مباريات اليوم"),
        "tz": ZoneInfo("Africa/Cairo"),
    },
    {
        "label": "Ajel",
        "home": "https://ajel.sa/sport",
        "keywords": ("مباريات اليوم", "القنوات الناقلة"),
        "tz": TZ,
    },
)
...
STATIC_CONFIRMATION_PAGES = (
    {
        "label": "Yallakora Matches Center",
        "url": "https://www.yallakora.com/matches-center",
        "tz": ZoneInfo("Africa/Cairo"),
        "per_day": "https://www.yallakora.com/matches-center?date={m:02d}/{d:02d}/{y}",
    },
    {
...
def parse_daily_article(url, source_tz=None, fallback_day=None):
    ...
    for row in soup.find_all("tr"):
        ...
        events.append({
            "channel": channel,
            "start": make_dt(day, time_match.group(1), time_match.group(2), source_tz),
            "title": title,
            "source": url,
    ...
    for source in EXTRA_SOURCES:
        ...
        for url in urls[:6]:
            ...
            for event in found:
                ...
                rows.append(event)
            if found:
                log(f"{source['label']} rows from {url}: {len(found)}")
...
def write_xml(events):
    ...
    def time_text(event):
        # وقت XMLTV يبقى بتوقيت المصدر (الرياض) مع الإزاحة، وTiviMate يحوّله
        # لتوقيت الجهاز. أما الوصف فيعرض التوقيتات الثلاثة صراحةً.
...
def inject_verified_fallback_fixtures(events):
    ...
    """
    verified = [
        # Verified: Al-Diriyah vs Al-Nassr, King Cup R32, 18 Aug 2026, 21:00 KSA, Thmanyah 1.
        {
            "date": date(2026, 8, 18),
            "start": datetime(2026, 8, 18, 21, 0, tzinfo=TZ),
            "title": "الدرعية - النصر",
            "channel": 1,
            "source": "verified fallback: Goal/365Scores/FilGoal",
            "confirmed": True,
        },
    ]
    existing_keys = {
...
def main():
    log("THMANYAH FINAL FIXED | VERIFIED FALLBACK | OCR CHANNEL-ONLY | GOAL/KOOORA KICKOFF | NO DIRECT OCR EVENTS | FIXTURE-DAY DEDUPE")
