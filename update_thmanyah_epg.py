...
def write_xml(events):
    ...
    while day_cursor < window_end:
        ...
        # Remove duplicate fixture/time records. Prefer confirmed/numbered copy.
        unique = {}
        ...
        # Every kickoff becomes a Guide-bar event.
        # IMPORTANT:
        # Stop at the NEXT kickoff. For the final match, stop after 3 hours.
        ...
        for index, event in enumerate(day_events):
            ...
            # If two different fixtures have exactly the same kickoff,
            # XMLTV cannot show two programmes simultaneously on one Guide row.
            # Combine them into one title instead of creating overlap.
            if segment_stop <= segment_start:
                ...
                # Only the first same-time event writes the combined programme.
                if same_time and event is same_time[0]:
                    combined_title = " / ".join(
                        e["title"] for e in same_time
                    )

                    # Find the next later kickoff, or use +3h.
                    later_starts = [
                        e["start"]
                        for e in day_events
                        if e["start"] > segment_start
                    ]
                    ...
                    if later_starts:
                        ...
                        combined_stop = min(
                            segment_start + timedelta(hours=3),
                            day_stop,
                        )

                    if combined_stop > segment_start:
                        add_guide_programme(
                            segment_start,
                            combined_stop,
                            combined_title,
        ...
        # Fill AFTER the final match, starting only after its real Guide segment.
