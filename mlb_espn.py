...
def _day(session, day: datetime) -> list[dict]:
    ...
    for event in data.get("events") or []:
        ...
        # STILL TO COME, and with a real first pitch. A game already
        # played is not a live event, and a game the feed has not timed
        # ("time TBD" days exist in the postseason bracket) has no
        # instant to print.
...
def collect(session, floor: datetime, ceiling: datetime) -> list[dict]:
    ...
    log(f"  MLB: {len(out)} game(s) with a network and a first pitch")
    return out
