...
def write_xml(events):
    ...
    def add_filler(number, channel_id, gap_start, gap_stop):
        ...
        while cursor < gap_stop:
            ...
            # IMPORTANT:
            # Give every Alwan channel a UNIQUE current programme title.
            # This avoids ten channels having an identical filler programme.
            ET.SubElement(
                programme,
                "title",
                {"lang": "ar"},
            ).text = f"ألوان الرياضية {number} — لا توجد مباراة حالياً"

            ET.SubElement(
                programme,
                "title",
                {"lang": "en"},
            ).text = f"Alwan Sport {number} — No match currently"
    ...
    for number in channel_numbers:
        ...
        for s, e in merged:
            if s > cursor:
                add_filler(number, channel_id, cursor, s)
            cursor = max(cursor, e)

        if cursor < window_end:
            add_filler(number, channel_id, cursor, window_end)
    ...
    # Hard validation: exactly ten Alwan EPG channels and each one must
    # have a programme covering the current Riyadh time.
    root = ET.parse(OUT).getroot()

    channel_ids = [channel.get("id") for channel in root.findall("channel")]
    expected_ids = {f"AlwanSports{number}" for number in range(1, 11)}

    if set(channel_ids) != expected_ids or len(channel_ids) != 10:
        raise RuntimeError(
            "Alwan XML validation failed; expected exactly 10 channels, got: "
            + ", ".join(channel_ids)
        )

    current = NOW.astimezone(TZ)
    coverage = {number: False for number in range(1, 11)}

    for programme in root.findall("programme"):
        channel_id = programme.get("channel") or ""
        match = re.fullmatch(r"AlwanSports(10|[1-9])", channel_id, re.I)
        if not match:
            continue
        ...
        if start_dt <= current < stop_dt:
            coverage[int(match.group(1))] = True

    log(
        "ALWAN CURRENT COVERAGE | "
        + ", ".join(
            f"{number}:{'YES' if coverage[number] else 'NO'}"
            for number in range(1, 11)
        )
    )

    missing = [number for number, ok in coverage.items() if not ok]
    if missing:
        raise RuntimeError(
            "Alwan XML validation failed; no current programme for channels: "
