...
# Length of each generated "no match" filler block, in hours.
FILLER_HOURS = 1
...
def write_xml(events):
    ...
    def add_filler(number, channel_id, gap_start, gap_stop):
        ...
        while cursor < gap_stop:
            stop = min(cursor + timedelta(hours=FILLER_HOURS), gap_stop)
    ...
    try:
        ...
        validate_xml(tmp, channel_ids_by_number)
        os.replace(tmp, OUT)
...
def validate_xml(path, channel_ids_by_number):
    ...
    current_coverage = {channel.get("id"): False for channel in actual_channels}
    programme_counts = {channel.get("id"): 0 for channel in actual_channels}
    spans = {channel.get("id"): [] for channel in actual_channels}
    ...
    missing = [
        channel_id for channel_id, ok in current_coverage.items() if not ok
    ]
