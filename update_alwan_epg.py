#!/usr/bin/env python3
"""
Alwan Sports XMLTV updater — channels 1 through 10.

Source:
  https://t.me/s/AlwanSports

Rules:
  • Always defines Alwan Sports channels 1..10 in the XML.
  • Extracts a programme only when Telegram explicitly shows:
      - two teams
      - a valid time
      - an explicit Alwan channel number 1..10
  • Never guesses a channel number.
  • Ignores Telegram placeholders/headings.
  • Keeps a rolling 1-day-back / 7-days-forward EPG window.
  • Preserves valid existing future programmes.
  • Does NOT touch Thmanyah files.
"""
...
def channel_number_from_id(channel_id):
    # Backward compatibility with the older channel-1 ID.
    if channel_id == "AlwanSports":
        return 1

    m = re.fullmatch(r"AlwanSports(10|[1-9])", channel_id or "", re.I)
    if m:
...
def write_xml(events):
    ...
    # Always publish all 10 channel definitions.
...
def main():
    ...
    # Always rewrite so channel definitions are guaranteed to be 1..10.
    write_xml(merged)
    log(f"Written: {OUT}")
