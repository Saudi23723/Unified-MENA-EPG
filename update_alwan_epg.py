...
def write_xml(events):
    ...
    for n in range(1, 11):
        ...
        ET.SubElement(
            ...
            "display-name",
            {"lang": "ar"},
        ).text = f"ألوان الرياضية {n}"

        ET.SubElement(
            ch,
            "display-name",
            {"lang": "en"},
        ).text = f"Alwan Sports {n}"

    channels_with_real = set()

    for e in events:
        n = int(e["channel"])
        if n not in VALID_CHANNELS:
            continue

        channels_with_real.add(n)
        cid = f"AlwanSports{n}"
        ...
        ET.SubElement(
            ...
            "desc",
            {"lang": "ar"},
        ).text = "مباراة منقولة على قنوات ألوان الرياضية"

    # Placeholder only for channels that have no real programmes,
    # so TiviMate can expose all 10 channels in Assign EPG.
    placeholder_start = (NOW + timedelta(days=6)).replace(
        hour=4,
        minute=0,
        second=0,
        microsecond=0,
    )
    placeholder_stop = placeholder_start + timedelta(minutes=30)

    for n in range(1, 11):
        if n in channels_with_real:
            continue
        ...
        p = ET.SubElement(
            ...
            "programme",
            {
                "start": placeholder_start.strftime("%Y%m%d%H%M%S %z"),
                "stop": placeholder_stop.strftime("%Y%m%d%H%M%S %z"),
                "channel": cid,
            },
        )
