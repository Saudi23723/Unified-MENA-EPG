...
# ---------------------------------------------------------------------------
# ترجمة أسماء الفرق (إنجليزي) - للاستخدام في المباريات المباشرة والفواصل
# ---------------------------------------------------------------------------
TEAM_DISPLAY_EN = {
    "الهلال": "Al-Hilal",
    "النصر": "Al-Nassr",
    ...
    def add_filler(number, channel_id, gap_start, gap_stop, next_event):
        ...
        for start, stop in chunks:
            if next_event is None or FILLER_MODE == "simple":
                ...
                # نضيف الترجمة الإنجليزية للفريقين في عنوان الفاصل، مع بقاء الأوقات
                title = f"المباراة القادمة - {times_str} - {bilingual_fixture_title(next_event['title'])}"
                description = (
