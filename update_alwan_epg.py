...
def write_xml(events):
    ...
    def add_filler(number, channel_id, gap_start, gap_stop):
        ...
        while cursor < gap_stop:
            ...
            ET.SubElement(
                ...
                "title",
                {"lang": "ar"},
            ).text = f"ألوان الرياضية {number} — لا توجد مباريات مجدولة"

            ET.SubElement(
                programme,
                "title",
                {"lang": "en"},
            ).text = f"Alwan Sport {number} — No scheduled matches"
            ...
            ET.SubElement(
                ...
                "لا توجد مباراة معلنة على هذه القناة في هذا الوقت."
            )
    ...
    # Validate the actual XML that TiviMate will read.
    root = ET.parse(OUT).getroot()
    channel_ids = [c.get("id") for c in root.findall("channel")]
    expected_ids = [f"AlwanSports{n}" for n in range(1, 11)]

    if channel_ids != expected_ids:
        raise RuntimeError(
            "Alwan XML channel definitions are wrong. Got: "
            + ", ".join(channel_ids)
        )

    programme_counts = {n: 0 for n in range(1, 11)}
    current_coverage = {n: False for n in range(1, 11)}
    now_check = NOW.astimezone(TZ)

    for programme in root.findall("programme"):
        channel_id = programme.get("channel") or ""
        m = re.fullmatch(r"AlwanSports(10|[1-9])", channel_id, re.I)
        if not m:
            continue

        number = int(m.group(1))
        programme_counts[number] += 1
        ...
        if start_dt <= now_check < stop_dt:
            current_coverage[number] = True

    log(
        "ALWAN XML PROGRAMMES | "
        + ", ".join(
            f"{n}:{programme_counts[n]}"
            for n in range(1, 11)
        )
    )
    ...
    log(
        ...
        + ", ".join(
            f"{n}:{'YES' if current_coverage[n] else 'NO'}"
            for n in range(1, 11)
        )
    )

    missing = [n for n, ok in current_coverage.items() if not ok]
    if missing:
        raise RuntimeError(
            "Missing current EPG coverage for Alwan channels: "
