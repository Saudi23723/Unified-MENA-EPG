...
def write_xml(events):
    """
    Clean XMLTV output for TiviMate:
    - Exactly 10 Alwan channels.
    - Exactly one stable ID per channel.
    - Real matches on the correct channel.
    - Continuous filler elsewhere so TiviMate does not show "No information".
    """
    ...
    # One clean channel definition per Alwan channel.
    for number in channel_numbers:
        ...
        ET.SubElement(
            ...
            "display-name",
            {"lang": "en"},
        ).text = f"Alwan Sport {number} 4K"

    # Real programmes grouped by channel.
    real_by_channel = {number: [] for number in channel_numbers}

    for event in events:
        number = int(event["channel"])
        real_by_channel.setdefault(number, []).append(event)
    ...
    # Continuous filler from start of today through the forward window.
    ...
    window_end = (
        window_start
        + timedelta(days=KEEP_DAYS_FORWARD + 1)
    )
    ...
    for number in channel_numbers:
        ...
        # Merge overlapping real programmes.
        merged = []
        ...
        for start, stop in merged:
            if start > cursor:
                ...
                ET.SubElement(
                    ...
                    "لا توجد مباراة معلنة على "
                    "هذه القناة في هذا الوقت."
                )
