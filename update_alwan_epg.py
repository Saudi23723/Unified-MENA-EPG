...
# Show the kickoff clock inside the "التالي" block title.
#
# Disabled: that clock is in the SOURCE timezone (Asia/Riyadh), while the
# guide draws the block in the device's own timezone, so the two numbers
# disagree on screen. The information is not lost - the block ends exactly
# when the match starts, TiviMate labels that edge in local time, and the
# full kickoff time stays in the block description.
# The ⏰ icon is kept either way.
SHOW_NEXT_MATCH_TIME = False

# Label appended to every match block. Set to "" to turn it off.
#
# It marks the broadcast as live rather than counting down, so it is also
# visible when you browse ahead in the guide - the standard EPG meaning.
LIVE_LABEL = "🔴 LIVE"

# Arabic forces right-to-left layout, so anything Latin placed at the END of
# a title is rendered at the far LEFT. That is how the clock and LIVE end up
# on the left with the Arabic on the right, without translating any names.
LRM = "\u200e"
...
# "Al-Riyadh (الرياض)" -> "الرياض". Cleans up bilingual titles left behind by
# an earlier version, so they are not treated as different matches.
BILINGUAL_RE = re.compile(r"[A-Za-z][A-Za-z0-9.\-'’ ]*\s*\(\s*([^()]+?)\s*\)")

LIVE_SUFFIX_RE = re.compile(r"\s*[·—\-]?\s*(?:🔴|⏺|●)?\s*LIVE\s*$", re.I)


def strip_decorations(title):
    """
    Undo everything write_xml adds to a title, so a title read back from the
    XML compares equal to the same fixture freshly parsed from Telegram.

    Without this, "هال - مان يونايتد 🔴 LIVE" and "هال - مان يونايتد" count as
    two different matches and the merge duplicates them on every run.
    """
    value = (title or "").replace(LRM, "").replace("\u200f", "")
    value = BILINGUAL_RE.sub(r"\1", value)
    value = LIVE_SUFFIX_RE.sub("", value)
    return norm(value)
...
def read_existing():
    ...
    for programme in root.findall("programme"):
        ...
        title = strip_decorations(title_node.text)
...
def write_xml(events):
    ...
    for number in CHANNEL_NUMBERS:
        for key, label_template in VARIANTS:
            ...
            # Alwan is one brand across all its channels, and publishes no
            # per-channel mark, so every channel carries the network avatar.
    ...
    def format_when(moment, reference_day):
        """Latin-only, so it can be pushed to the left edge of the title."""
        clock = moment.strftime("%H:%M")

        if reference_day is not None and moment.date() == reference_day:
            return clock

        weekday = EN_WEEKDAYS[moment.weekday()]
    ...
    def add_filler(number, channel_id, gap_start, gap_stop, next_event):
        ...
        for start, stop in chunks:
            if next_event is None or FILLER_MODE == "simple":
                ...
                # Arabic first, Latin last -> the icon and any clock render
                # on the far left.
                marker = f"⏰ {when}" if SHOW_NEXT_MATCH_TIME else "⏰"
                title = f"التالي: {next_event['title']} {ltr(marker)}"

                description = (
                    f"المباراة القادمة على Alwan Sport {number}:\n"
                    f"{next_event['title']}\n"
                    f"{ltr(format_when(next_event['start'], None))}"
                )
