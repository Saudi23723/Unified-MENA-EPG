...
def alwan_aliases(number):
    # Canonical + exact/common names seen in TiviMate playlists.
    return [
        f"AlwanSports{number}",
        f"AlwanSport{number}",
        f"Alwan Sport {number}",
        f"Alwan Sports {number}",
        f"Alwan Sport {number} 4K",
        f"Alwan Sports {number} 4K",
    ]
...
def write_xml(events):
    ...
    # Define multiple compatible IDs/names for every Alwan channel.
    for number in channel_numbers:
        for channel_id in alwan_aliases(number):
    ...
    real_by_channel = {n: [] for n in channel_numbers}
    ...
    for event in events:
        ...
        # Duplicate the same real programme under every compatible ID.
        for channel_id in alwan_aliases(number):
    ...
    for number in channel_numbers:
        intervals = []
        for event in sorted(real_by_channel.get(number, []), key=lambda x: x["start"]):
