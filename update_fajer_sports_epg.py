...
def collect_sofascore_day(day):
    """
    External fixture corroboration.

    SofaScore is NEVER allowed to change the Fajer channel
    or kickoff time. It can only provide a title, and only
    under a high-confidence match rule.
    """
    url = SOFASCORE_DAILY.format(day=day.isoformat())
    ...
    log(
        f"SofaScore fixtures {day.isoformat()}: "
        f"{len(out)}"
    )
...
def _best_sofascore_title(ocr_event, fixtures):
    """
    Safe rules:
...
def enrich_titles(events, text_hints):
    ...
    for ev in generic_events:
        ...
        day = ev["start"].astimezone(PALESTINE).date()

        if day not in sofa_cache:
            sofa_cache[day] = collect_sofascore_day(day)

        resolved = _best_sofascore_title(
            ev,
            sofa_cache[day],
        )
...
def main():
    log(
        ...
        "SAFE TITLE MATCH: Telegram first, SofaScore fallback | "
        "channels 1-5 | "
        "UTC XMLTV | "
