...
CYAN_MIN_HEIGHT_RATIO = 0.010
CYAN_MAX_HEIGHT_RATIO = 0.30
...
def _drop_outlier_boxes(boxes):
    """
    The height ceiling is deliberately generous so that a poster with
    only one or two matches -- whose strips are very tall -- is still
    read. Once at least three cards are present, anything wildly off
    the median card height cannot be a card.
    """
    if len(boxes) < 3:
        return boxes

    median_height = _median([b[3] - b[1] for b in boxes])

    return [
        box
        for box in boxes
        if 0.5 * median_height <= (box[3] - box[1]) <= 2.0 * median_height
    ]
...
def _rescue_header_boxes(arr, body, header_candidates, w):
    """
    A crowded poster (9, 12, 14 matches...) pushes its first card up
    into the top band that is normally skipped as the header. Such a
    card is recovered only when it matches the column position AND the
    height of the confirmed cards below it -- the title banner never
    does, so the header itself can never be mistaken for a match.
    """
    if not body or not header_candidates:
        return []

    median_height = _median([b[3] - b[1] for b in body])

    rescued = []

    for box in header_candidates:
        x0, y0, x1, y1 = box
        height = y1 - y0

        if not (0.6 * median_height <= height <= 1.6 * median_height):
            continue

        # Must line up with an ACTUAL column of confirmed cards. Matching
        # the median instead used to accept a centred title banner on a
        # two-column poster, because the median of two columns lands
        # between them.
        aligned = any(
            abs(x0 - other[0]) <= 0.05 * w
            and abs(x1 - other[2]) <= 0.05 * w
            for other in body
        )
        ...
        # Final gate: it must actually be two-toned like a match strip.
...
def _strips_from_mask(mask, h, w, row_factor, col_factor=0.45):
    """
    Return (card strips, header-band candidates).

    Nothing here assumes how MANY matches a poster carries: every cyan
    band that survives the shape tests becomes a card.
    """
    row_hits = mask.sum(1)
    ...
    for y0, y1 in bands:
        ...
        # The poster header ("تشاهدون اليوم" + date) is held back rather
        # than discarded, and reconsidered only if it looks exactly like
        # a real card.
        in_header = y0 < HEADER_SKIP_RATIO * h
        ...
        # A single card is one cyan strip made of two tones. Only a
        # hairline seam left by re-encoding is glued here. A real gap
        # between two side-by-side cards must stay a gap; genuine
        # fragments are recovered later by the guarded repair pass.
        merged = []
        gap_limit = max(2, int(0.008 * w))
        ...
        # A big white clock or channel glyph eats the cyan coverage of the
        # columns it covers, which used to clip the card and cut the time
        # half off. Grow each run outward while columns are still cyan.
        edge_factor = max(0.10, col_factor * 0.25)
        limit = len(col_hits)

        for x0, x1 in merged:
            while x0 > 0 and col_hits[x0 - 1] >= edge_factor * band_h:
                x0 -= 1

            while x1 < limit and col_hits[x1] >= edge_factor * band_h:
                x1 += 1

            if in_header:
                header_boxes.append((x0, y0, x1, y1))
            else:
                boxes.append((x0, y0, x1, y1))

    return (
        _collapse_overlaps(boxes),
        _collapse_overlaps(header_boxes),
    )
...
def detect_match_strips(im):
    ...
    for name, mask, row_factor, col_factor in attempts:
        boxes, header_boxes = _strips_from_mask(
            mask,
            h,
            w,
            row_factor,
        ...
        boxes = _drop_outlier_boxes(boxes)

        rescued = _rescue_header_boxes(arr, boxes, header_boxes, w)

        if rescued:
            log(
                "STRIP DETECTION | "
                f"{len(rescued)} card(s) recovered from the header band"
            )
...
def _confusion_key(name):
    ...
    return "".join(
        ...
        for c in _club_key(name)
    )


def _closest_club(key):
    """
    Score against the confusion-folded form. A dotted-letter slip scores
    near 1.0; a different club scores low no matter how similar it looks
...
def correct_club_name(name):
    ...
    if len(words) > 1:
        score, match = _closest_club(" ".join(words[1:]))
...
def _pick_team_name(votes):
    ...
    return correct_club_name(winner)
...
def read_teams(im, arr, box):
    ...
    for part_box in (right_box, left_box):
        ...
        names.append(_pick_team_name(votes))
