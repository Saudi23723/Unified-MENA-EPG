...
def _cyan_mask(arr):
    ...
    re-compression and small palette changes still work.
    """
    r = arr[..., 0]
    g = arr[..., 1]
    b = arr[..., 2]
...
def detect_match_strips(im):
    ...
    row_hits = mask.sum(1)
    bands = _runs(
        row_hits >= CYAN_MIN_WIDTH_RATIO * w * 0.55,
...
def read_channel(im, arr, box, split_x):
    ...
    The half contains two white tokens: the Arabic word
    'قناة' (wide) and the digit (narrow). We isolate the
    tokens by column projection and only OCR the narrow one,
    so Arabic letters can never be misread as digits.
    """
    x0, y0, x1, y1 = box
    ...
    for start, end in tokens:
        ...
        # a single numeral is narrow; 'قناة' is >1.5x its height
        if width > 1.35 * text_height:
            continue
        ...
        for psm in (10, 7):
            ...
            if len(got) == 1 and got.isdigit():
                candidates.append(int(got))
                break
...
def read_teams(im, box):
    """
    Team names live in the dark-navy strip directly above
    the cyan strip: [ team A ][ logo ][ team B ].
    Arabic is right-to-left, so the RIGHT block is listed first.
    """
    x0, y0, x1, y1 = box

    strip_h = y1 - y0

    ty0 = max(0, int(y0 - strip_h * 1.15))
    ty1 = y0 - 1
    ...
    for part in (right, left):
        ...
        for psm in (7, 6):
            for invert in (False, True):
                ...
                cand = clean_candidate(_ocr_norm(raw))

                if cand and _team_line_ok(cand):
    ...
    return (
        f"{_clean_team_display(names[0])} - "
        f"{_clean_team_display(names[1])}",
        0.95,
    )
...
def parse_poster(im, post_time, source):
    ...
    for index, box in enumerate(boxes, 1):
        ...
        try:
            title, confidence = read_teams(im, box)
