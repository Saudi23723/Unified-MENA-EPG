...
TELEGRAM_HUGE_BEFORE = 999999999999
...
def fetch_recent_telegram_pages(base):
    ...
    seed_urls = [
        _telegram_url(base),
        _telegram_url(base, before=TELEGRAM_HUGE_BEFORE),
    ]
    ...
    for url in seed_urls:
        try:
            ...
            warn(f"Telegram page fetch failed {url} | {exc}")
            continue

        ids, times = _telegram_page_meta(html)
        ...
        if ids:
            page_min = min(ids)
            oldest_cursor = page_min if oldest_cursor is None else min(oldest_cursor, page_min)
    ...
    while oldest_cursor and len(pages) < TELEGRAM_MAX_PAGES:
        url = _telegram_url(base, before=oldest_cursor)
        ...
        if not ids or signature in seen_signatures:
            break
        ...
        page_min = min(ids)
        if page_min >= oldest_cursor:
            break
        oldest_cursor = page_min
...
def parse_telegram_pages(pages):
    ...
    for html in pages:
        _, times = _telegram_page_meta(html)
        if times:
...
def main():
    log(
        ...
        "FINAL ONE-SHOT | MULTIPAGE CACHE-SAFE TELEGRAM + POSTER OCR + SAFE TITLE MATCH | "
        "channels 1-5 | "
        "UTC XMLTV | "
