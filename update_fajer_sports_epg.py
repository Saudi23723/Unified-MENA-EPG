...
def _rescue_header_boxes(body, header_candidates, w):
    """
    ...
    median_x0 = _median([b[0] for b in body])
    median_x1 = _median([b[2] for b in body])
    ...
    for box in header_candidates:
        ...
        if abs(x0 - median_x0) > 0.05 * w:
            continue

        if abs(x1 - median_x1) > 0.05 * w:
            continue
...
def detect_match_strips(im):
    ...
    for name, mask, row_factor, col_factor in attempts:
        ...
        rescued = _rescue_header_boxes(boxes, header_boxes, w)
