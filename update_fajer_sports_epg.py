...
def read_channel(im, arr, box, split_x):
    ...
    for start, end in ordered:
        ...
        if width > 2.0 * text_height:
            continue

        # '1' is the only numeral this narrow in the poster font,
        # and it is exactly the glyph Tesseract misreads most.
        # Geometry is decisive here, so trust it over OCR.
        if width <= 0.45 * text_height:
            return 1
...
def _sanitize_team_name(raw):
    """
    The team strip is Arabic-only. Anything else (digits,
    Latin letters, logo fragments read as 'سسس' or '333')
    is OCR noise from the club badges, so drop it.
    """
    ...
    for word in text.split():
        ...
        # badge fragments often come out as 'سس' / 'اا'
        if len(set(word)) == 1:
            continue
...
def read_teams(im, box):
    ...
    the cyan strip: [ team B ][ logo ][ team A ].
