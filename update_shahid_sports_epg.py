...
def parse_livesoccertv():
    ...
    for node in soup.find_all(["h2", "h3", "h4", "h5", "tr", "li"]):
        ...
        start = make_dt(row_day, time_value[0], time_value[1], VEGAS_TZ)
        if not in_window(start):
            continue
    ...
    if not events:
        ...
        for i, line in enumerate(lines):
            ...
            start = make_dt(current_day, time_value[0], time_value[1], VEGAS_TZ)
            if in_window(start):
                events.append({
                    "start": start,
                    "title": title,
...
def main():
    log("SHAHID FINAL | 365Scores + Goal + Kooora + LiveSoccerTV + LiveFootballTV + Bundesliga Official Match Pages | NO OCR")
