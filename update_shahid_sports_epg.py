...
# - Description shows the whole day's schedule in Abu Dhabi + Las Vegas time
# -----------------------------------------------------------------------------
...
BUNDESLIGA_FIXTURES = "https://products.bundesliga.com/fixtures"
...
def make_dt(day, hour, minute, source_tz=RIYADH_TZ):
    local = datetime(day.year, day.month, day.day, int(hour), int(minute), tzinfo=source_tz)
    return local.astimezone(RIYADH_TZ)
...
def event_group_key(event):
    ...
    return (event["start"].date().isoformat(), title_signature(event["title"]))
...
def parse_livesoccertv():
    ...
    if not events:
        ...
        for i, line in enumerate(lines):
            ...
            start = make_dt(current_day, time_value[0], time_value[1])
            if in_window(start):
                events.append({
                    "start": start,
                    "title": title,
...
def parse_bundesliga_official():
    """
    Official Bundesliga Media fixtures table.

    IMPORTANT:
    We use the page's explicit UTC kick-off column, NOT scraped local-page
    display times. This avoids the timezone/date bug that previously placed
    every Matchday 1 game on 28 August at the same time.
    """
    events = []

    def add_utc(day, hour, minute, title, source_url):
        start_utc = datetime(
            day.year, day.month, day.day,
            int(hour), int(minute),
            tzinfo=timezone.utc,
        )
        start = start_utc.astimezone(RIYADH_TZ)
        if in_window(start):
            events.append({
                "start": start,
                "title": title,
    ...
    # Bundesliga Matchday fixtures from official Bundesliga Media table.
    try:
        soup = BeautifulSoup(fetch(BUNDESLIGA_FIXTURES), "html.parser")
        ...
        for row in rows:
            ...
            # Expected columns:
            # Matchday | Day | Date | Kick-off (CET/CEST) | Kick-off (UTC) |
            # Home Team | Away Team | ...
            ...
            dm = re.fullmatch(r"(\d{1,2})-([A-Za-z]{3})-(\d{4})", date_text)
            tm = re.fullmatch(r"(\d{1,2}):(\d{2})", utc_text)
            if not dm or not tm or not home or not away:
                continue
            ...
            mon = month_map.get(dm.group(2))
            if not mon:
                continue

            day = date(int(dm.group(3)), mon, int(dm.group(1)))
            add_utc(
                day,
                int(tm.group(1)),
                int(tm.group(2)),
        ...
        warn(f"Bundesliga official fixtures failed: {exc}")

    # Franz Beckenbauer Supercup: official local kickoff 20:30 Germany
    # = 18:30 UTC on 22 Aug 2026.
    try:
        soup = BeautifulSoup(fetch(BUNDESLIGA_PRESEASON), "html.parser")
        page = norm(soup.get_text(" ", strip=True)).casefold()
        if (
            "22 august" in page
            and "borussia dortmund" in page
            and "bayern" in page
            and ("8.30pm" in page or "8:30pm" in page)
        ):
            add_utc(
                date(2026, 8, 22),
                18, 30,
        ...
        warn(f"Bundesliga official Supercup failed: {exc}")
...
def collect_all_sources():
    ...
    # Daily Arabic schedule tables.
    for label, home in (
        ("365Scores", SCORES365_HOME),
        ("Goal", GOAL_HOME),
        ("Kooora", KOOORA_HOME),
    ):
...
def write_xml(events):
    tv = ET.Element("tv", {"generator-info-name": "Shahid Sports Guide FINAL v7"})
    ...
    def dual_time(event):
        abu = event["start"].astimezone(ABU_DHABI_TZ)
        vegas = event["start"].astimezone(VEGAS_TZ)
        return f"{abu:%H:%M} أبو ظبي | {vegas:%H:%M} لاس فيغاس"
    ...
    def day_description(day, day_events):
        ...
        for event in sorted(day_events, key=lambda x: (x["start"], x["title"])):
            lines.append(f"{dual_time(event)} | {event['title']}")
        return "\n".join(lines)
    ...
    for p in root.findall("programme"):
        if p.get("channel") != CHANNEL_ID:
            continue
        s = datetime.strptime((p.get("start") or "")[:14], "%Y%m%d%H%M%S").replace(tzinfo=RIYADH_TZ)
        e = datetime.strptime((p.get("stop") or "")[:14], "%Y%m%d%H%M%S").replace(tzinfo=RIYADH_TZ)
        slots.append((s, e))
...
def main():
    log("SHAHID FINAL v7 | 365Scores + Goal + Kooora + LiveSoccerTV + LiveFootballTV + Bundesliga Official UTC | NO OCR")
    ...
    for event in fresh:
        ...
        vegas = event["start"].astimezone(VEGAS_TZ)
        log(
            f"  SHAHID GUIDE | {abu:%Y-%m-%d %H:%M} أبو ظبي | "
            f"{vegas:%Y-%m-%d %H:%M} لاس فيغاس | "
