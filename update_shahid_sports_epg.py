...
def nearby_structural_block(node):
    """
    Return the SMALLEST local block that simultaneously contains:
      - a Shahid/MBC marker
      - a usable kickoff time
      - a valid fixture

    We intentionally refuse huge ancestors because they can contain an
    unrelated Shahid mention elsewhere on the page.
    """
    candidates = [node]
    parent = getattr(node, "parent", None)

    # Only climb two levels. Never fall back to the whole article/body.
    for _ in range(2):
        if parent is None:
            break
    ...
    for candidate in candidates:
        visible = norm(candidate.get_text(" ", strip=True))
        marker_text = shahid_marker_text(candidate)

        # Reject huge mixed-content blocks.
        if len(visible) > 1800 or len(marker_text) > 5000:
            continue

        if not has_shahid_marker(marker_text):
            continue

        if not extract_time(visible):
            continue

        if not fixture_from_text(visible):
            continue

        return candidate, marker_text

    return None, ""


def local_shahid_fixture(node):
    """
    Strictly validate that Shahid marker + fixture + time live in the same
    compact structural block. Returns (block_node, marker_text, title, time)
    or None.
    """
    block_node, marker_text = nearby_structural_block(node)

    if block_node is None:
        return None

    visible = norm(block_node.get_text(" ", strip=True))
    title = fixture_from_text(visible)
    time_value = extract_time(visible)

    if not title or not time_value:
        return None

    # Final guard against author/date metadata or score-like false positives.
    parts = [part.strip() for part in title.split(" - ", 1)]
    if len(parts) != 2 or not all(looks_like_team_name(part) for part in parts):
        return None

    return block_node, marker_text, title, time_value
...
def parse_daily_article(url, label):
    """
    Strict Shahid match extraction.

    Accept only when Shahid/MBC marker, kickoff time and fixture are all found
    inside the SAME compact HTML row/card. Page-wide mentions are ignored.
    """
    try:
    ...
    # Also include elements whose attributes explicitly identify Shahid/MBC.
    for node in soup.find_all(True):
        ...
        if has_shahid_marker(attrs_text):
    ...
    for node in candidate_nodes:
        ...
        parsed = local_shahid_fixture(node)
        if not parsed:
            continue

        block_node, marker_text, title, time_value = parsed

        visible = norm(block_node.get_text(" ", strip=True))
        block_day = parse_date(visible, day) or day
        ...
        log(
            f"{label} SHAHID STRICT | "
            f"{start:%Y-%m-%d %H:%M} | {title}"
        )

    # Conservative text fallback:
    # Only use a short window where the Shahid token itself is present.
    if not events:
        lines = [
            norm(x)
        ...
        for i, line in enumerate(lines):
            if not has_shahid_marker(line):
                continue

            block_lines = lines[max(0, i - 2):min(len(lines), i + 3)]
...
def read_existing():
    """
    Shahid guide is rebuilt from fresh source data on EVERY run.

    We intentionally do not preserve old real events because a false positive
    from an earlier parser version could otherwise survive forever.
    """
    return []

def merge_existing(existing, fresh):
    # Fresh source data only. Existing XML is intentionally ignored.
    return dedupe([
        event
        for event in fresh
...
def main():
    log('SHAHID SPORTS GUIDE v4 | fresh rebuild + strict local Shahid detection | Abu Dhabi + Las Vegas')
    existing = read_existing()
    log('Existing Shahid programmes kept: 0 (fresh rebuild mode)')
