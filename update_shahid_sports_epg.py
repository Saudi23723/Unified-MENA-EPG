...
def discover_daily_articles(home_url, label):
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if label in ("Goal", "Kooora"):
            ...
            wanted = (
                "القنوات الناقلة لمباريات اليوم" in combined
                or "مباريات اليوم" in combined
                or "بث مباشر مباريات اليوم" in combined
            )
    ...
    limit = 12 if label in ("Goal", "Kooora") else 30
...
def write_xml(events):
    tv = ET.Element("tv", {"generator-info-name": "Shahid Sports Guide FINAL v5"})
...
def main():
    log("SHAHID FINAL v5 | 365Scores + Goal + Kooora + LiveSoccerTV + LiveFootballTV | NO OCR")
