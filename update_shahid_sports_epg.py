...
BUNDESLIGA_MD1 = "https://www.bundesliga.com/en/bundesliga/matchday/2026-2027/1"
...
def parse_bundesliga_official():
    ...
    def add_event(day, hour, minute, title, source_url):
        local = datetime(
            day.year, day.month, day.day,
            int(hour), int(minute),
            tzinfo=BERLIN_TZ,
        )
        start = local.astimezone(RIYADH_TZ)
        if in_window(start):
            events.append({
                "start": start,
                "title": title,
    ...
    try:
        soup = BeautifulSoup(fetch(BUNDESLIGA_MD1), "html.parser")
        seen = set()
        for anchor in soup.find_all("a", href=True):
            href = urljoin(BUNDESLIGA_MD1, anchor.get("href"))
            if "/en/bundesliga/matchday/2026-2027/1/" not in href:
                continue
            base = re.sub(r"/(?:liveticker|lineup|stats|news)/?$", "", href)
            if base in seen:
                continue
            seen.add(base)
            ...
            block = norm(parent.get_text(" ", strip=True))
            ...
            if not title:
                ...
                if "-vs-" in slug:
                    ...
                    def pretty(x):
                        ...
                        return names.get(x, " ".join(w.capitalize() for w in x.split()))

                    title = f"{pretty(a)} - {pretty(b)}"

            if not title:
                continue

            day = parse_date(block, date(2026, 8, 28))
            tv = extract_time(block)
            if day and tv:
                add_event(day, tv[0], tv[1], title, BUNDESLIGA_MD1)

    except Exception as exc:
        warn(f"Bundesliga official Matchday 1 failed: {exc}")
    ...
    try:
        ...
        if (
            "22 august" in low
            and "borussia dortmund" in low
            and "bayern" in low
            and ("8.30pm" in low or "8:30pm" in low)
        ):
            add_event(
                date(2026, 8, 22),
                20, 30,
...
def write_xml(events):
    tv = ET.Element("tv", {"generator-info-name": "Shahid Sports Guide FINAL v6"})
...
def main():
    log("SHAHID FINAL v6 | 365Scores + Goal + Kooora + LiveSoccerTV + LiveFootballTV + Bundesliga Official | NO OCR")
