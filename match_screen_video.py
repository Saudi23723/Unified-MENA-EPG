...
SCREENS = {
    ...
    # TWENTY SECONDS, the same as channels 1 and 2. The 14 paid for the
    # pages five rows a page cost; this channel carries eight on the
    # now-and-next table now (see ON_A_PAGE in turkish_ppv_epg.py),
    # which is ten boards instead of fifteen, and ten at 14s would give
    # a viewer 1.8s a row — less reading time per row than this channel
    # has ever had. At 20s it is 2.5s a row and the day still comes
    # round in 3:20.
