...
CANDIDATES: dict[str, list[str]] = {
    ...
    # Deliberately not listed. The forty Qatar marks are drawn by
    # make_bein_bubbles.py, not downloaded, and this script writes only the
    # keys it holds, so leaving them out is what stops a future run from
    # painting over all forty.
    #
    # They were fetched here until the drawing replaced them, and the
    # reason they had to stop being fetched is in what the upstream
    # database actually had: no separate picture for beIN SPORTS 9, for
    # AFC 4-6, or for XTRA 3-9, so eleven channels were served another
    # channel's mark — the seven XTRA ones read "XTRA 1" in plain words.
    # A drawn plate always names its own channel.
    #
    # bein_wordmark is the flat official mark the bubbles are drawn over,
    # and bein_tr1-5 / bein_trmax1-2 are Türkiye's copies of it. All are
    # held in this repository for the same reason, and bein_haber, which
    # is Türkiye-only and unchanged, is still fetched above.
}
