...
# EIGHT ROWS A PAGE, the same as channels 1 and 2, and the same 20s a
# page — "٨ صفوف افضل صح؟", after the three boards were rendered on
# this channel's own published rows and looked at side by side.
#
# It was FIVE for a good reason that stopped being true. On the info
# board eight rows are 56px each and every size in them falls to the
# drawing's smallest step, which is what "it's too small" meant off a
# television across a room, twice; five rows are 89px and the fixture
# reaches 34px. That arithmetic belongs to the info board. This channel
# wears the now-and-next table now, and there the comparison runs the
# other way round, because the two boards disagree about WHICH HALF of
# a row matters.
#
# This channel's titles are "Yarış 2", "Çeyrek Finaller", "Ispanya GP"
# — race 2, the quarter-finals, the Spanish GP. None of them says
# anything on its own; the competition beside it is the half a viewer
# reads. The info board printed the title at 30px and the competition
# UNDER it at 17px, the useful half in the small type. The table bolds
# the competition and sets the title after it, so at eight rows that
# half is 26px: half again the size it is published at today, not
# smaller.
#
# AND THE PAGE GOES BACK TO 20 SECONDS (see SCREENS in
# match_screen_video.py). The 14 was bought to pay for the extra pages
# five rows cost — 74 rows over fifteen boards. Eight rows is ten
# boards, and ten boards at 14s would give a viewer 1.8s a row, less
# reading time per row than this channel has ever had. At 20s it is
# 2.5s a row and the whole day still comes round in 3:20, which is what
# fifteen pages of five took.
ON_A_PAGE = 8
