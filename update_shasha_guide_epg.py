...
def _mentions_shasha(text):
    t = norm(text).casefold()
    return any(marker in t for marker in SHASHA_MARKERS)


def _article_date(text):
    t = norm(text)
...
def _normalize_ar_time_marker(marker):
    marker = norm(marker).casefold()
    if marker in ("م", "pm", "مساء", "مساءً", "عصر", "عصرًا", "ظهرا", "ظهرًا"):
        return "pm"
    if marker in ("ص", "am", "صباح", "صباحًا", "صباحا"):
        return "am"
    return ""


def _extract_makkah_time(text):
    """
    Parse Saudi/Makkah time from 365Scores/Goal/Kooora rows.

    Examples:
      22:45 السعودية، 23:45 الإمارات
      01:00 ظهرًا بتوقيت القاهرة ومكة
      4:00 بتوقيت القاهرة 5:00 بتوقيت مكة المكرمة
      9:45م بتوقيت القاهرة – 10:45م بتوقيت مكة المكرمة
    """
    t = norm(text)

    # First priority: time explicitly attached to مكة/السعودية.
    patterns = [
        r"(\d{1,2}):(\d{2})\s*(م|ص|pm|am|مساءً|مساء|عصرًا|عصر|ظهرًا|ظهرا|صباحًا|صباحا|صباح)?"
        r"\s*(?:بتوقيت\s*)?(?:مكة(?:\s*المكرمة)?|السعودية)",
        r"(?:مكة(?:\s*المكرمة)?|السعودية)\s*[:\-]?\s*"
        r"(\d{1,2}):(\d{2})\s*(م|ص|pm|am|مساءً|مساء|عصرًا|عصر|ظهرًا|ظهرا|صباحًا|صباحا|صباح)?",
    ]
    ...
    # 365Scores often writes Cairo first and Makkah second without punctuation.
    if not matches and ("مكة" in t or "السعودية" in t):
        pos = max(t.rfind("مكة"), t.rfind("السعودية"))
        if pos > 0:
            before = t[:pos]
    ...
    hh = int(m.group(1))
    mm = int(m.group(2))
    marker = _normalize_ar_time_marker(m.group(3) or "")

    if marker == "pm" and 1 <= hh <= 11:
        hh += 12
...
def _extract_match_title(text):
    ...
    # Remove time / broadcaster tail to stop team 2 from swallowing metadata.
    cut_patterns = [
        r"\s+\d{1,2}:\d{2}",
        r"\s+\|\s+",
    ]
    ...
    for pat in patterns:
        ...
        if any(x in a for x in bad):
            continue
        if any(x in b for x in ("القنوات", "المعلق", "الموعد")):
            continue
...
def _date_in_rolling_window(day):
    now = datetime.now(RIYADH)
    d = datetime(day.year, day.month, day.day, 12, 0, tzinfo=RIYADH)
    return (
...
def _discover_links(label, index_urls, max_links=120):
    """
    Crawl site's own index pages and collect likely daily broadcaster articles.
    We intentionally allow several naming styles used by 365Scores/Goal/Kooora.
    """
    keywords = (
        "القنوات-الناقلة",
        "القنوات_الناقلة",
    ...
    for index_url in index_urls:
        try:
            ...
            warn(f"{label} index failed: {index_url} | {exc}")
            continue
    ...
    log(f"{label}: discovered {len(found)} candidate articles")
    return found


def _rows_from_table(table):
    """
    Yield reconstructed logical rows.

    Goal often places multiple broadcaster rows under a single fixture using
    rowspan. We carry forward match/time until a new fixture appears.
    """
    current_title = None
    current_time = None
    ...
    for tr in table.find_all("tr"):
        ...
        # Search each cell independently first, then whole row.
        for cell in cells:
            title = title or _extract_match_title(cell)
            time_value = time_value or _extract_makkah_time(cell)

        title = title or _extract_match_title(row_text)
        time_value = time_value or _extract_makkah_time(row_text)
        ...
        yield {
            ...
            "time": time_value or current_time,
        }


def _parse_article(label, url):
    try:
        r = requests.get(url, headers=HEADERS, timeout=25)
        r.raise_for_status()
    except Exception as exc:
    ...
    for sel in ("h1", "title"):
        ...
        if n:
            heading_parts.append(norm(n.get_text(" ", strip=True)))

    day = _article_date(" ".join(heading_parts))
    if day is None:
        day = _article_date(norm(soup.get_text(" ", strip=True))[:2500])

    if day is None or not _date_in_rolling_window(day):
        return []
    ...
    for table in soup.find_all("table"):
        for item in _rows_from_table(table):
            if not _mentions_shasha(item["text"]):
                continue
            if not item["title"] or not item["time"]:
                continue

            hh, mm = item["time"]
            start = datetime(day.year, day.month, day.day, hh, mm, tzinfo=RIYADH)
    ...
    # --- FALLBACK TEXT STATE MACHINE ---
    # Handles pages where the rendered article isn't a semantic HTML table.
    current_title = None
    current_time = None

    blocks = soup.select(
        "p, li, article div, .article-body div, .content div, section div"
    )
    ...
    for node in blocks:
        ...
        title = _extract_match_title(raw)
        tm = _extract_makkah_time(raw)
        ...
        if not _mentions_shasha(raw):
            continue

        if not current_title or not current_time:
            continue
    ...
    if events:
        log(f"{label}: {len(events)} SHASHA fixtures from {url}")

    return events


def _scrape_article_source(label, index_urls, max_links=120):
    links = _discover_links(label, index_urls, max_links=max_links)
    events = []

    for url in links:
        events.extend(_parse_article(label, url))

    events = dedupe(events)
    log(f"{label}: {len(events)} SHASHA fixtures from article crawl")
    return events
...
def scrape_all_sources():
    """
    One-pass SHASHA guide builder.

    Source order:
      1) LiveFootballTV
      2) LiveSoccerTV
      3) 365Scores Arabic daily broadcaster articles
      4) Goal Arabic daily broadcaster articles
      5) Kooora daily broadcaster articles
      6) SHASHA official website
      7) SHASHA Sports X

    Instagram is intentionally skipped because GitHub Actions is getting HTTP 429.
    Every fixture must be explicitly tied to SHASHA/شاشا by a source.
    """
    events = []
