...
def scrape_all_sources():
    """
    Merge multiple independent sources.
    Rules:
      - No source may guess kickoff time.
      - Kooora/Goal/365Scores only count a fixture if SHASHA/شاشا is explicitly
        mentioned in the same matched container.
      - Duplicate fixtures are merged.
    """
    events = []

    # Dedicated TV/broadcast listing sources.
    events.extend(scrape_livesoccertv())
    events.extend(scrape_livefootballtv())

    # Broad sports/news sources filtered by explicit SHASHA mention.
    events.extend(scrape_365scores())
    events.extend(scrape_kooora())
    events.extend(scrape_goal())

    # SHASHA-owned sources.
