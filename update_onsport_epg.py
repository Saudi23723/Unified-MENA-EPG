...
def build_day_description(channel_name: str, d: date, events: list[dict]) -> str:
    ...
    for ev in sorted(events, key=lambda x: x["start"]):
        ...
        lines.append(f"• {ev['home']} - {ev['away']} — {ev['competition']}")
        lines.append(f"  {source_time:%H:%M} بتوقيت القاهرة (وقت المصدر)")
...
def write_xml(events: list[dict]) -> None:
    ...
    today_source = utc_now().astimezone(CAIRO).date()
    ...
    for ev in events:
        d = ev["start"].astimezone(CAIRO).date()
    ...
    for channel_id, cfg in CHANNELS.items():
        for off in range((last_day - first_day).days + 1):
            ...
            day_start_source = datetime(d.year, d.month, d.day, 0, 0, tzinfo=CAIRO)
            day_end_source = day_start_source + timedelta(days=1)
            day_start = day_start_source.astimezone(UTC)
            day_end = day_end_source.astimezone(UTC)
...
def main():
    log(
        "ON SPORT FINAL vTIMEFIX | FilGoal Egyptian League assignments + EPL official validation "
        "+ channel-specific LiveFootballTV | SOURCE CAIRO TIME | UTC XMLTV | "
        "TIVIMATE AUTO-CONVERT | NO ABU DHABI/LAS VEGAS | 1 + 2 + MAX + PLUS"
    )
