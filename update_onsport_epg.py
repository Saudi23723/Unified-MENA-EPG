...
def write_xml(events: list[dict]) -> None:
    ...
    today_source = utc_now().astimezone(ABU_DHABI).date()
    ...
    for channel_id, cfg in CHANNELS.items():
        for off in range((last_day - first_day).days + 1):
            ...
            for ev in day_events:
                ...
                next_start = day_end
                current_index = day_events.index(ev)
                if current_index + 1 < len(day_events):
                    next_start = day_events[current_index + 1]["start"].astimezone(UTC)
